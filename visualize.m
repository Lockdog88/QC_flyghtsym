...
function animate(data, model, thrusts, plots)
    ...
    for t = 1:2:length(data.t)
        ...
        subplot(plots(3));
        multiplot2(data, data.x, t);
        xlabel('Time (s)');
        ylabel('Height');
        title('Vertical Movement (PID)');
    end
end
...
function multiplot(data, values, ind)
    ...
    plot(times, values(1, :), 'b-', times, values(2, :), 'r', times, values(3, :), 'g.');
...
% Plot three components of a vector in RGB.
function multiplot2(data, values, ind)
    % Select the parts of the data to plot.
    times = data.t(:, 1:ind);
    values = values(:, 1:ind);

    % Plot in RGB, with different markers for different components.
    plot(times, values(3, :), 'b');
    % Set axes to remain constant throughout plotting.
    xmin = min(data.t);
    xmax = max(data.t);
    ymin = 0;
    ymax = max(data.x(3,:))*1.2;
    axis([xmin xmax ymin ymax]);
end
...
function [h thrusts] = quadcopter()
    ...
    y = 0.5 * y;
    z = 0.5 * z;
    h(3) = surf(x - 5, y, z, 'EdgeColor', 'none', 'FaceColor', 'black');
    h(4) = surf(x + 5, y, z, 'EdgeColor', 'none', 'FaceColor', 'black');
    h(5) = surf(x, y - 5, z, 'EdgeColor', 'none', 'FaceColor', 'black');
    h(6) = surf(x, y + 5, z, 'EdgeColor', 'none', 'FaceColor', 'black');

    % Draw thrust cylinders.
    [x y z] = cylinder(0.1, 7);
    thrusts(1) = surf(x, y + 5, z, 'EdgeColor', 'none', 'FaceColor', 'g');
    thrusts(2) = surf(x + 5, y, z, 'EdgeColor', 'none', 'FaceColor', 'b');
    thrusts(3) = surf(x, y - 5, z, 'EdgeColor', 'none', 'FaceColor', 'g');
    thrusts(4) = surf(x - 5, y, z, 'EdgeColor', 'none', 'FaceColor', 'b');
...
function h = prism(x, y, z, w, l, h)
    ...
    for i = 1:size(faces, 1)
        h(i) = fill3(X(faces(i, :)), Y(faces(i, :)), Z(faces(i, :)), 'y'); hold on;
    end
