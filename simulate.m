...
%     input: [4xN double]
%        dt: 0.0050
function [result controller_params] = simulate(controller, tstart, tend, dt)
    % Physical constants.
    g = 9.81;
    m = 2.5;
    L = 0.385;
    k = 3e-6;
    b = 1e-7;
    I = diag([3.660e-3, 3.660e-3, 7.32e-3]);
    kd = 0.25;
    ...
    for t = ts
        ...
        if nargin == 0
            ...
            [i, controller_params] = controller(controller_params, thetadot, error);
        end
        ...
        inputout(:, ind) = i;
        %errorout(:,ind) = error;
        %errout(:,ind) = controller_params.err;
    end

    % Put all simulation variables into an output struct.
    result = struct('x', xout, 'theta', thetaout, 'vel', xdotout, ...
                    'angvel', thetadotout, 't', ts, 'dt', dt, 'input', inputout, 'error', errorout, 'err', errout);
end
